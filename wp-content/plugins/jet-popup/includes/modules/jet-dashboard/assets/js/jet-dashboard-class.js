...
class JetDasboardClass {
	...
	initVueComponents() {
		...
		Vue.component( 'jet-dashboard-header', {
			...
			computed: {
				...
				videoLink: function() {
					...
					return this.configDefined && this.config['videoLink'] && visibleMap.includes( this.licenseType ) ? this.config['videoLink'] : false;
				},
			}
		...
		Vue.component( 'plugin-item-more', {
			...
			data: function() {
				return {
					morePluginsUrl: window.JetDashboardConfig.getMorePluginsUrl || {},
				}
			},

			computed: {
				demoLink: function() {
					let authorSlug = window.JetDashboardConfig.themeInfo.authorSlug || 'jet-dashboard';
					...
					if ( areaConfig && areaConfig.hasOwnProperty( 'banners' ) ) {
						bannersData = this.avaliableBanners.filter( ( avaliableBanner ) => {
							return areaConfig.banners.includes( avaliableBanner.id )
								&& ( '' === avaliableBanner.licenseType || avaliableBanner.licenseType === this.licenseType );
						} );
					}
