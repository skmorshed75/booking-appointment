	...
	Vue.component( 'welcome-page', {
		...
		computed: {
			...
			wizardSectionsVisible: function() {
				let licenseType = this.$root.licenseType,
					visibleMap  = [
						'plugin-set',
						'all-inclusive',
						'lifetime',
				...
				return ( !this.isWizardInstalled || !this.isWizardActivated ) && visibleMap.includes( licenseType );
			},
			...
			extrasVisible: function() {
				...
				return 0 !== this.avaliableExtras.length && visibleMap.includes( licenseType ) && ! this.wizardSectionsVisible;
			},
			...
			getMoreBannerVisible: function() {
				...
				return visibleMap.includes( licenseType );
			},
			...
			getMoreBannerLink: function() {
				let baseUrl = this.generalConfig.pricingPageUrl || 'https://crocoblock.com/pricing/',
					utmString = window.JetDasboard.getUtmParamsString( {
						utm_source: 'jet-dashboard',
						utm_medium: this.$root.licenseType || 'license-not-active',
						utm_term: this.themeInfo ? this.themeInfo.theme : 'unknown-theme',
						utm_campain: 'get-jetplugins-bundle',
					} );
			...
			isLifetime: function() {
				...
				return 'lifetime' === licenseType ? true : false;
			}
		},
