...
class Form {
	...
	public function __construct() {
		...
		add_action(
			'jet-engine/forms/booking/notifications/available_macros_after',
	...
	public function parse_appointments_macros( $message_content, $class_instant ) {
		$message_content = preg_replace_callback( '/%(.*?)(\|([a-zA-Z0-9\(\)_-]+))?%/', function( $match ) use ( $class_instant ) {
			switch ( $match[1] ) {
				...
				/*case 'appointment_date':
					$field_name = $this->get_field_value( $class_instant->handler->form_fields, 'appointment_date', 'name' );
					$dete      =  $class_instant->data[ $field_name ];
				break;*/
